...
// Builds a shop_name (normalized) -> { area manager, region } lookup from Map rows.
export function buildMapIndex(mapRows) {
  ...
  for (const row of mapRows) {
    ...
    index.set(normalizeName(shop), { manager, region, shop });
  }
  return index;
}
...
// Joins Stock rows (which already carry SHOP_NAME directly) against the Map
// index, producing one output row per stock row with the resolved area
// manager and region. groupBy is either "manager" or "region".
export function joinStockData(stockRows, mapRows, groupBy = "manager") {
  const mapIndex = buildMapIndex(mapRows);

  const results = [];
  for (const row of stockRows) {
    const shopRaw = (row.SHOP_NAME ?? "").toString().trim();
    let manager = null;
    let region = null;
    let status = "ok";
    ...
    if (!shopRaw) {
      ...
      const mapped = mapIndex.get(normalizeName(shopRaw));
      if (mapped) {
        manager = mapped.manager;
        region = mapped.region || UNASSIGNED;
      } else {
        status = "no_manager"; // shop not found in the Map
      }
    }
    ...
    results.push({
      ...
      custPrice: Number(row.CUST_PRICE ?? 0) || 0,
  ...
  const key = groupBy === "region" ? "region" : groupBy === "all" ? null : "manager";
  const grouped = new Map();
  if (key === null) {
    grouped.set("All Stock", results);
  } else {
    for (const r of results) {
      const k = r[key];
